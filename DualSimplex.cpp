...
void CreatSimplexTable(vector < vector<float> > &SimplexTable, float *Z, float *CN, float *CB, string *bv, string *nbv, int n, int m, int slack_var, vector < vector<float> > &NewMatrix, float *b, float *Optim, float d)
{
	int i,j;
	float sum;
...
void PrimalSimplex(int n, int m, vector < vector<float> > &STable, string bv[], string nbv[], float Z[], float CN[], float CB[], bool &condn, float *ratio)
{
	number_Iteration++;
	int i, j, k=0,p=0;
	float pivot, sum, x;
...
void DualSimplex(int n, int m, vector < vector<float> > &STable, string bv[], string nbv[], float Z[], float CN[], float CB[], bool &condn, float *ratio)
{
	number_Iteration++;
	int i, j, k=0,p=0;
	float pivot, sum, x;
...
int main()
{
	...
	for (i=0;i<m;i++)
	{
		if (SimplexTable[i][n] < 0)
		{			
			print_simplex(n, m, SimplexTable, bv, nbv, Z, CN, CB);
			cout << endl;
			DualSimplex(n, m, SimplexTable, bv, nbv, Z, CN, CB, condn, ratio);
			i=-1;
		}
		if (condn == true)
		{
	...
	for (i=0;i<n;i++)
	{
		if (Z[i] < 0)
		{
			PrimalSimplex(n, m, SimplexTable, bv, nbv, Z, CN, CB, condn, ratio);
			i=-1;
		}
		if (condn == true)
		{
	...
	for (i=0;i<m;i++)
	{
		int fraction = SimplexTable[i][n] - (int)SimplexTable[i][n];
		if (fraction != 0)
		{
			cout << fraction << endl;
		}
